...
<!-- #NAVIGATION -->
<c:import url="/sidebar?menu=${portal_code}"/>
<!-- END NAVIGATION -->
                ...
                <div class="panel panel-default">
                    <div class="panel-heading btn-toolbar">
                        <ul class="nav nav-tabs pull-left">
                            ...
                            <li id="btnCreate">
                                <a href="javascript:"><span>新增${corp_type}</span></a>
                            </li>
                        </ul>
                        ...
                        <div class="table-responsive">
                            <table id="table" class="table table-striped table-hover no-margin">
                                ...
                                <c:if test="${page.totalCount>0}">
                                    <tbody>
                                    <c:forEach items="${page.rows}" var="corporation">
                                        <tr>
                                            <td class="text-left name"><c:out value="${corporation.name}"/></td>
                                            <td class="text-left"><c:out value="${corporation.province_name}${corporation.city_name}"/></td>
                                            <td class="text-left"><c:out value="${corporation.contact_phone}"/></td>
                                            <td class="text-left"><c:out value="${corporation.address}"/></td>
                                            <td class="text-center">
                                                <div class="lk-btn-group" data-id="${corporation.id}">
...
<script type="text/javascript">
    function doQuery(pageNo, pageSize) {
        location.assign("${home}/corporation/${portal_code}/list?" + $.param({
                    "keyword": $("#keyword").val(),
                    "pageNo": pageNo,
                    "pageSize": pageSize
                }));
    ...
    function doCreate() {
        location.assign("${home}/corporation/${portal.code}/create");
    }

    function doUpdate(id) {
        location.assign("${home}/corporation/${portal.code}/update?id=" + id);
    }

    function doDelete(id) {
        $.ServiceClient.invoke("${home}/corporation/${portal.code}/delete/" + id + ".json", {
    ...
    $(function () {
        $(document).on("click", "#btnQuery", function (e) {//查询
            ...
            var $this = $(this);
            layer.confirm("确认删除${corp_type}[" + getName($this) + "]？", {
                icon: 3,
                title: '提示'
            }, function (index) {
