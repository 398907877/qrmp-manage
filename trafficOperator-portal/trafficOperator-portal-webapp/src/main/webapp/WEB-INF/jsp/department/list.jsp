                ...
                <div class="panel panel-default">
                    <div class="panel-heading btn-toolbar">
                        <ul class="nav nav-tabs pull-left in">
                            ...
                            <li id="btnCreate">
                                <a href="javascript:"><span>新增部门</span></a>
                            </li>
                        </ul>
                        ...
                        <div class="table-responsive">
                            <table id="table" class="table table-striped table-hover no-margin">
                                ...
                                <c:if test="${page.totalCount>0}">
                                    <tbody>
                                    <c:forEach items="${page.rows}" var="department">
                                        <tr>
                                            <td class="text-left">${department.priority}</td>
                                            <td class="text-left">
                                                <span class="tree-ctrl" data-id="${department.id}" data-loaded="false">
                                                <span class="tree-fork ${department.hasChildren?"fork-close":""}"></span>
                                                <span class=""></span>
                                                </span>
                                            ...
                                            <td class="text-left"><c:out value="${department.parent_name}"/></td>
                                            <td class="text-center">
                                                <div class="lk-btn-group" data-id="${department.id}">
...
<script type="text/javascript">
    function doQuery(pageNo, pageSize) {
        location.assign("${home}/department/list?" + $.param({
                    "keyword": $("#keyword").val(),
                    "pageNo": pageNo,
                    "pageSize": pageSize
                }));
    ...
    function doCreate(parent_id) {
        location.assign("${home}/department/create?parent_id=" + (parent_id || 0));
    }

    function doUpdate(id) {
        location.assign("${home}/department/update?id=" + id);
    }

    function doDelete(id, callback) {
        $.ServiceClient.invoke("${home}/department/delete/" + id + ".json", {
    ...
    function doExpand(parent_id, index, callback) {
        $.ServiceClient.invoke("${home}/department/find.json", {
            data: {
    ...
    $(function () {
        var colorClasses = ["blue", "green", "orange", "teal", "red", "magenta", "pink"];
            ...
            inModel: [
                {
                    ...
                    formatter: function (row) {
                        return '<span class="txt-color-' + colorClasses[row.level % colorClasses.length] + '">' + row.priority + '</span>';
                    }
                }, {
                    align: "left",
                    formatter: function (row) {
                        ...
                        return '<span class="tree-ctrl' + clazz + '" data-id="' + row.id + '" data-loaded="false"><span class="tree-indent">' +
        ...
        $(document).on("click", "#btnQuery", function (e) {//查询
            ...
            var $this = $(this);
            layer.confirm("确认删除部门[" + getName($this) + "]？", {
                icon: 3,
                title: '提示'
            }, function (index) {
