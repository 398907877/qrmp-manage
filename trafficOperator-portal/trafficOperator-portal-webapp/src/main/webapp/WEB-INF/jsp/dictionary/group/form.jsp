                    ...
                    <div class="panel-heading btn-toolbar">
                        <ul class="nav nav-tabs pull-left">
                            ...
                            <li id="btnCreate" class="active">
                                <a href="javascript:"><span>${!empty group.id?"修改":"新增"}分组</span></a>
                            </li>
                        </ul>
                        ...
                        <form id="oForm" class="form-horizontal" role="form">
                            ...
                            <fieldset>
                                <div class="form-group">
                                    <label class="control-label col-sm-2 required">分组名称</label>
                                    <div class="col-sm-10">
                                        <input type="text" id="name" name="name" class="form-control" maxlength="20"
                                               placeholder="请输入分组名称" value="<c:out value="${group.name}"/>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="control-label col-sm-2 required">分组编码</label>
                                    <div class="col-sm-10">
                                        <input type="text" id="code" name="code" class="form-control" maxlength="20"
                                               placeholder="请输入分组编码" value="<c:out value="${group.code}"/>">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" id="priority" name="priority" class="form-control"
                                               maxlength="4"
                                               placeholder="请输入自然数排序序号，系统将会根据排序从小到大进行排列" value="${group.priority}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea id="remarks" name="remarks" class="form-control" maxlength="500"
                                                  placeholder="请输入备注"><c:out value="${group.remarks}"/></textarea>
                                    </div>
                                </div>
                            </fieldset>
                            <div class="form-group">
...
<script type="text/javascript">
    function doSave() {
        $.ServiceClient.invoke("${home}/dictionary/group/${!empty group.id?"update.json":"create.json"}", {
            data: {
                "id": parseInt($("#id").val()),
                "name": $.trim($("#name").val()),
                "code": $.trim($("#code").val()),
    ...
    function doBack(){
        $.goBack("${referer}", "${home}/dictionary/group/list");
    }
        ...
        return $('#oForm').validate({
            rules: {
                ...
                code: {
                    required: true,
                    unique: {
                        url: "${home}/dictionary/group/check_code.json",     //后台处理程序
                        type: "get",                                     //数据发送方式
            ...
            messages: {
                ...
                code: {required: "请输入分组编码", unique: "编码已经存在"},
