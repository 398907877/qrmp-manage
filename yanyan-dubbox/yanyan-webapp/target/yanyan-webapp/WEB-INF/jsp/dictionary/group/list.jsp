                ...
                <div class="panel panel-default">
                    <div class="panel-heading btn-toolbar">
                        <ul class="nav nav-tabs pull-left">
                            ...
                            <li id="btnCreate">
                                <a href="javascript:"><span>新增分组</span></a>
                            </li>
                        </ul>
                        ...
                        <div class="table-responsive">
                            <table id="table" class="table table-striped table-hover no-margin">
                                ...
                                <c:if test="${page.totalCount>0}">
                                    <tbody>
                                    <c:forEach items="${page.rows}" var="group">
                                        <tr>
                                            <td class="text-left">${group.priority}</td>
                                            <td class="text-left name"><c:out value="${group.name}"/></td>
                                            <td class="text-left"><c:out value="${group.code}"/></td>
                                            <td class="text-left"><c:out value="${group.remarks}"/></td>
                                            <td class="text-center">
                                                <div class="lk-btn-group" data-id="${group.id}">
...
<script type="text/javascript">
    function doQuery(pageNo, pageSize) {
        location.assign("${home}/dictionary/group/list?" + $.param({
                    "keyword": $("#keyword").val(),
                    "pageNo": pageNo,
                    "pageSize": pageSize
                }));
    ...
    function doCreate() {
        location.assign("${home}/dictionary/group/create");
    }

    function doUpdate(id) {
        location.assign("${home}/dictionary/group/update?id=" + id);
    }

    function doDelete(id) {
        $.ServiceClient.invoke("${home}/dictionary/group/delete/" + id + ".json", {
    ...
    $(function () {
        $(document).on("click", "#btnQuery", function (e) {//查询
            ...
            var $this = $(this);
            layer.confirm("确认删除分组[" + getName($this) + "]？", {
                icon: 3,
                title: '提示'
            }, function (index) {
                doDelete(getId($this));
